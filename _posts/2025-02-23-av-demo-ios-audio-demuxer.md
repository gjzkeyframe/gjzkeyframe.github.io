---
title: iOS AVDemo（4）：音频解封装，代码开源并提供解析
description: 介绍 iOS 音频解封装的流程和原理，并提供 Demo 源码和解析。
author: Keyframe
date: 2025-02-23 07:38:08 +0800
categories: [音视频源码示例]
...
在上面的`拷贝下一份音频/视频采样数据`接口中，我们使用的是依然 [CMSampleBufferRef](https://developer.apple.com/documentation/coremedia/cmsamplebufferref/ "CMSampleBufferRef") 作为返回值类型。在这个接口中我们通过 `CMSampleBufferRef` 打包的是从 MP4/M4A 文件解封装后得到的 AAC 编码数据。
